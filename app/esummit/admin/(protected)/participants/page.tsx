...
import { FiDownload, FiSearch, FiRefreshCw, FiExternalLink, FiCheckCircle, FiXCircle, FiTrash2, FiUserPlus } from 'react-icons/fi';
import * as XLSX from 'xlsx';
import toast from 'react-hot-toast';
import { updateProfileRole, deleteProfile, promoteToCA } from '@/app/actions/registrations';
import { useTransition } from 'react';
import { FiLoader, FiEye } from 'react-icons/fi';
import ParticipantDetailsModal from '@/components/esummit/admin/ParticipantDetailsModal';
...
export default function ParticipantsPage() {
    ...
    const handleDeleteProfile = async (profileId: string) => {
        if (!confirm('Are you SURE you want to delete this user? This will remove them from Auth and Profiles. This action is IRREVERSIBLE.')) return;
    ...
    const handlePromoteToCA = async (profileId: string, fullName: string, college?: string) => {
        if (!confirm(`Register ${fullName} as a Campus Ambassador?`)) return;
    ...
    return (
        <div className="space-y-6">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-800/50 border-b border-gray-700">
                                ...
                                <th className="p-4 text-gray-400 font-medium text-xs uppercase tracking-wider text-center">Actions</th>
                                <th className="p-4 text-gray-400 font-medium text-xs uppercase tracking-wider">User Type</th>
                                <th className="p-4 text-gray-400 font-medium text-xs uppercase tracking-wider">System</th>
                                        ...
                                        <td className="p-4">
                                            <div className="flex items-center justify-center gap-2">
                                                <button
                                                    ...
                                                    className="p-2 bg-esummit-primary/10 text-esummit-primary rounded-lg hover:bg-esummit-primary hover:text-white transition-all flex items-center gap-2 text-xs font-bold"
                                                    title="View Details"
                                                >
                                                    <FiEye />
                                                </button>
                                                ...
                                                <button
                                                    onClick={() => handleDeleteProfile(participant.id)}
                                                    disabled={updatingRoleId === participant.id || !!participant.user_type}
                                                    className={`p-2 rounded-lg transition-all ${!!participant.user_type ? 'bg-gray-800 text-gray-600 grayscale cursor-not-allowed' : 'bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white'}`}
            ...
            <ParticipantDetailsModal
                ...
                onPromoteToCA={handlePromoteToCA}
                onDeleteProfile={handleDeleteProfile}
                isUpdating={updatingRoleId === selectedParticipant?.id}
            />
        </div>
    );
}
